...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask_img, query_embed, pos_embed, query_mask):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src.shape
        src = src.flatten(2).permute(2, 0, 1)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)
        mask_img = mask_img.flatten(1)
        query_embed = query_embed.permute(1, 0, 2)  # from NxSxHD to SxNxHD
        query_mask = query_mask.permute(1, 0 ,2)

        memory = self.encoder(src, src_key_padding_mask=mask_img, pos=pos_embed)
        hs = self.decoder(query_embed, memory, memory_key_padding_mask=mask_img, pos=pos_embed, target_key_padding_mask=query_mask)
